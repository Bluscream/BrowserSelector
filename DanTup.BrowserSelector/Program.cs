...
namespace DanTup.BrowserSelector
{
    internal class Program
    {
        ...
        private static void ShowHelpInfo(string noArgumentReasonDescription = null)
        {
...
To open multiple urls at the same time and wait for them, try the following:
    ...
    BrowserSelector.exe ""url-or-file"" ""url-or-file"" --wait ""url-or-file""", "BrowserSelector", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private static void EnsureAdmin(string arg)
        {
        ...
        private static void LaunchUrlFile(string file, bool waitForClose = false)
        {
            string url = "";
            string[] lines;
        ...
        private static void LaunchBrowser(string url, bool waitForClose = false)
        {
            try
            {
                ...
                foreach (var preference in urlPreferences)
                {
                    ...
                    if (urlPattern.StartsWith("/") && urlPattern.EndsWith("/"))
                    {
                        ...
                        matches = Regex.IsMatch(domain, pattern);
                    }
                    else
                    {
                        ...
                        var glob = Glob.Parse(urlPattern);
                        matches = glob.IsMatch(domain);
        ...
        private static void CreateSampleSettings()
        {
